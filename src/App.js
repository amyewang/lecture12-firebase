...
class App extends React.Component {
  ...
  signUp(email, password, handle, avatar) {
    ...
    console.log("You want to sign up huh?");
    ...
    firebase.auth().createUserWithEmailAndPassword(email, password)
      .then((firebaseUser) => {
        ...
        //jsonObjectInTheCloud['users'][uid] = userData
        var newUserRef = firebase.database().ref('users/'+firebaseUser.uid)
        newUserRef.set(userData); //asynchronous firebase call

        return profilePromise;
      })
      .catch((error) => { //report any errors--useful!
        console.log(error);
      });
  }
